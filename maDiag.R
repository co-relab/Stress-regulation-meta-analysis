...
# dat %>% filter(strategy == 1) %$% paste(round(sum(is.na(.))/prod(dim(.))*100, 3), "%", sep = "") # insert collumn numbers
missmap(dat, rank.order = TRUE, margins = c(8, 0), legend = F)    # insert collumn numbers
...
#fit FE model to all possible subsets
gosh.plot <- gosh(ma.uni, progbar = TRUE, subsets = 1000, parallel = "multicore")
plot(gosh.plot, out = 7, breaks = 50) # Testing the influence of single outliers
